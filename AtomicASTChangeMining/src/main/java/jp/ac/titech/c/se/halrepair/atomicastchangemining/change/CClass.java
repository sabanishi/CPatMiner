...
import jp.ac.titech.c.se.halrepair.atomicastchangemining.utils.Pair;
import jp.ac.titech.c.se.halrepair.atomicastchangemining.utils.PairDescendingOrder;
...
public class CClass extends ChangeEntity {
    ...
    private final AbstractTypeDeclaration declaration;
    private final HashSet<String> superClassNames = new HashSet<String>();
    private final ArrayList<CField> fields = new ArrayList<CField>();
    private final ArrayList<CMethod> methods = new ArrayList<CMethod>();
    private final ArrayList<CInitializer> initializers = new ArrayList<CInitializer>();
    ...
    public CClass(CFile cFile, TypeDeclaration type, CClass outterClass) {
        ...
        if (type.getSuperclassType() != null) {
            String name = FileIO.getSimpleClassName(type.getSuperclassType().toString());
            superClassNames.add(name);
        }

        if (type.superInterfaceTypes() != null) {
            for (int i = 0; i < type.superInterfaceTypes().size(); i++) {
                String name = FileIO.getSimpleClassName(type.superInterfaceTypes().get(i).toString());
                superClassNames.add(name);
            }
        }
        ...
        if (type.getFields() != null) {
            for (FieldDeclaration field : type.getFields()) {
                for (int i = 0; i < field.fragments().size(); i++) {
                    ...
                    String fieldType = FileIO.getSimpleClassName(field.getType().toString());
        ...
        if (type.bodyDeclarations() != null) {
            ...
            for (int i = 0; i < type.bodyDeclarations().size(); i++) {
                ...
                if (type.bodyDeclarations().get(i) instanceof Initializer) {
                    initializers.add(new CInitializer(
                            this,
                            staticId,
    ...
    public double computeSimilarity(CClass otherClass, boolean inMapped) {
        ...
        mapAll(new HashSet<CClass>(
                        this.innerClasses),
                new HashSet<CClass>(otherClass.getInnerClasses(false)),
                mappedInnersM,
                mappedInnersN);
    ...
    public static void map(HashSet<CClass> classesM, HashSet<CClass> classesN, HashSet<CClass> mappedClassesM, HashSet<CClass> mappedClassesN) {
    ...
    public static void mapAll(HashSet<CClass> classesM,
                              HashSet<CClass> classesN,
                              HashSet<CClass> mappedClassesM,
    ...
    public boolean hasChangedName() {
        if (getCType() == Type.Unchanged) return false;
        if (getCType() == Type.Added || getCType() == Type.Deleted) return true;
        if (getCType() == Type.Modified) {
            if (this.mappedClass == null) return true;
