...
import jp.ac.titech.c.se.halrepair.atomicastchangemining.utils.StringProcessor;
...
public class CMethod extends ChangeEntity {
    ...
    private static final double THRESHOLD_SIGNATURE_SIMILARITY = 0.625;
    private static final double THRESHOLD_BODY_SIMILARITY = 0.75;
    @SuppressWarnings("unused")
    private static final double THRESHOLD_DIFFABILITY = 0.5;
    ...
    public static void map(HashSet<CMethod> methodsM,
                           HashSet<CMethod> methodsN,
                           HashSet<CMethod> mappedMethodsM,
                           HashSet<CMethod> mappedMethodsN,
                           boolean inMappedClasses) {
        ...
        for (CMethod cmM : methodsM) {
            ...
            for (CMethod cmN : methodsN) {
                ...
                boolean isMapped = (inMappedClasses && sim[0] >= THRESHOLD_SIGNATURE_SIMILARITY)
                        || (sim[0] > 0 && sim[1] == 1.0)
                        || (sim[0] >= THRESHOLD_SIGNATURE_SIMILARITY && sim[1] >= THRESHOLD_BODY_SIMILARITY);
                if (isMapped) {
    ...
    private double[] computeSimilarity(CMethod other, boolean inMappedClasses) {
        ...
        double body = 0;
        if (!this.vector.isEmpty() || !other.getVector().isEmpty()) {
    ...
    private double computeNameSimilarity(CMethod other, boolean inMappedClasses) {
        ...
        if (inMappedClasses
                && !this.cClass.getSimpleName().equals(other.cClass.getSimpleName())
    ...
    public void deriveChanges() {
        ...
        boolean matched = this.declaration.subtreeMatch(new ASTMatcher(false) {
            ...
            public boolean match(SimpleName node, Object other) {
                if (!(other instanceof SimpleName)) return false;
                return true;
            }

            @Override
            public boolean match(AnonymousClassDeclaration node, Object other) {
                if (!(other instanceof AnonymousClassDeclaration)) return false;
                return true;
            }
        }, cmN.declaration);
        if (matched) return;
