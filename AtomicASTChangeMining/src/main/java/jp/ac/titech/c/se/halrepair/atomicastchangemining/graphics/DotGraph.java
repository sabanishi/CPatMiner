package jp.ac.titech.c.se.halrepair.atomicastchangemining.graphics;
...
import jp.ac.titech.c.se.halrepair.atomicastchangemining.pdg.graph.PDGControlEdge;
import jp.ac.titech.c.se.halrepair.atomicastchangemining.pdg.graph.PDGControlNode;
import jp.ac.titech.c.se.halrepair.atomicastchangemining.pdg.graph.PDGDataEdge;
import jp.ac.titech.c.se.halrepair.atomicastchangemining.pdg.graph.PDGDataNode;
import jp.ac.titech.c.se.halrepair.atomicastchangemining.pdg.graph.PDGEntryNode;
    ...
    public DotGraph(PDGGraph pdg, boolean changeOnly) {
        ...
        for (PDGNode node : pdg.getNodes()) {
            ...
            if (node instanceof PDGEntryNode) {
                graph.append(addNode(id, node.getLabel(), SHAPE_ELLIPSE, STYLE_DOTTED, color, color));
            } else if (node instanceof PDGControlNode) {
                graph.append(addNode(id, node.getLabel(), SHAPE_DIAMOND, null, color, color));
            } else if (node instanceof PDGActionNode) {
                graph.append(addNode(id, node.getLabel(), SHAPE_BOX, null, color, color));
            } else if (node instanceof PDGDataNode) {
                graph.append(addNode(id, node.getLabel(), SHAPE_ELLIPSE, null, color, color));
            }
        }
        // add edges
        for (PDGNode node : pdg.getNodes()) {
            ...
            for (PDGEdge e : node.getInEdges()) {
                ...
                if (e instanceof PDGDataEdge) {
                    ...
                    graph.append(addEdge(sId, tId, STYLE_DOTTED, null, label + (((PDGDataEdge) e).getType() == PDGDataEdge.Type.PARAMETER ? n : "")));
