package jp.ac.titech.c.se.halrepair.atomicastchangemining.pdg.graph;
...
import jp.ac.titech.c.se.halrepair.atomicastchangemining.exas.ExasSingleFeature;
import jp.ac.titech.c.se.halrepair.atomicastchangemining.treed.TreedConstants;
import jp.ac.titech.c.se.halrepair.atomicastchangemining.utils.FeatureAscendingOrder;
...
public class PDGGraph implements Serializable {
			...
			if (statements.get(e) instanceof SwitchCase || e == statements.size() - 1) {
				...
				if (e > s + 1) {
					...
					if (sc.isDefault()) {
						...
						cg.mergeSequentialData(new PDGDataNode(dummy.astNode, dummy.astNodeType, dummy.key, dummy.dataType, dummy.dataName), PDGDataEdge.Type.REFERENCE);
	...
	private PDGGraph buildPDG(PDGNode control, String branch,
		...
		if (astNode.getOperator() == PrefixExpression.Operator.INCREMENT
			...
			pdg.mergeSequentialData(new PDGDataNode(node.astNode, node.astNodeType, node.key,
					node.dataType, node.dataName), PDGDataEdge.Type.DEFINITION);
		}
		return pdg;
	}
	...
	private PDGGraph buildPDG(PDGNode control, String branch,
		...
		pdg.mergeSequentialData(new PDGActionNode(control, branch,
				astNode, ASTNode.ASSIGNMENT, null, null, "="), PDGDataEdge.Type.PARAMETER);
		pdg.mergeSequentialData(var, PDGDataEdge.Type.DEFINITION);
		pdg.mergeSequentialData(new PDGDataNode(null, var.astNodeType, var.key, var.dataType, var.dataName), PDGDataEdge.Type.REFERENCE);
	...
	private void delete(PDGNode node) {
		if (statementSinks.contains(node))
			for (PDGEdge e : node.inEdges)
				if (e instanceof PDGDataEdge) {
					if (((PDGDataEdge) e).type == PDGDataEdge.Type.DEPENDENCE)
						statementSinks.add(e.source);
					else if (((PDGDataEdge) e).type == PDGDataEdge.Type.PARAMETER)
						sinks.add(e.source);
				}
		...
		if (statementSources.contains(node))
			for (PDGEdge e : node.outEdges)
				if (e instanceof PDGDataEdge
						&& ((PDGDataEdge) e).type == PDGDataEdge.Type.DEPENDENCE)
	...
	private PDGGraph buildArgumentPDG(PDGNode control, String branch,
		...
		if (rets.size() > 0) {
			...
			pdg.mergeSequentialData(new PDGDataNode(null, dummy.astNodeType,
					dummy.key, dummy.dataType, dummy.dataName), PDGDataEdge.Type.REFERENCE);
			return pdg;
		}
		...
		pdg.mergeSequentialData(new PDGActionNode(control, branch,
				null, ASTNode.ASSIGNMENT, null, null, "="), PDGDataEdge.Type.PARAMETER);
		...
		pdg.mergeSequentialData(new PDGDataNode(null, dummy.astNodeType, dummy.key,
				dummy.dataType, dummy.dataName), PDGDataEdge.Type.REFERENCE);
		return pdg;
	}
			...
			if (node instanceof PDGActionNode) {
				for (PDGNode preNode : preNodesOfNode.get(node)) {
					if (preNode instanceof PDGActionNode && !node.hasInNode(preNode) && ((PDGActionNode) node).hasBackwardDataDependence((PDGActionNode) preNode))
						new PDGDataEdge(preNode, node, PDGDataEdge.Type.DEPENDENCE);
				}
			}
		}
	}
	...
	private void pruneTemporaryDataDependence() {
		for (PDGNode node : nodes) {
			...
			while (i < node.inEdges.size()) {
				...
				if (e.source != entryNode && e instanceof PDGDataEdge && ((PDGDataEdge) e).type == PDGDataEdge.Type.DEPENDENCE) {
			...
			while (i < node.outEdges.size()) {
				...
				if (e.target != endNode && e instanceof PDGDataEdge && ((PDGDataEdge) e).type == PDGDataEdge.Type.DEPENDENCE) {
				...
				for (PDGEdge e1 : new HashSet<PDGEdge>(node.inEdges)) {
					for (PDGEdge e2 : new HashSet<PDGEdge>(e1.source.inEdges)) {
						if (e2 instanceof PDGDataEdge
								&& ((PDGDataEdge) e2).type == PDGDataEdge.Type.PARAMETER)
							delete(e2.source);
					}
					delete(e1.source);
				}
		...
		for (PDGNode node : nodes) {
			...
			if (astNode != null) {
				...
				if (otherAstNode != null) {
					...
					if (otherNode != null)
						new PDGDataEdge(otherNode, node, PDGDataEdge.Type.MAP);
				}
			}
		}
...
//		markChanges(entryNode.astNode);
	...
	private void addDefinitions(PDGDataNode node, HashMap<String, PDGNode> defs) {
		if (node.getDefinitions().isEmpty() && node.getQualifier() == null) {
			...
			new PDGDataEdge(def, node, PDGDataEdge.Type.REFERENCE);
			for (PDGEdge e : node.outEdges)
